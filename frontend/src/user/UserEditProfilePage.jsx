...
import { CheckCircle, CheckCircle2, PencilIcon } from "lucide-react";
...
const UserEditProfilePage = () => {
  ...
  //to update the profile with update date
  const handleSubmit = async (event) => {
    event.preventDefault();
    setError("");
    if (form.phone.trim().replace(/\D/g, "").length !== 10) {
      setError("Mobile number must be exactly of 10 digist.");
      return;
    }
    ...
    try {
      ...
      if (result.ok) {
        setToast("Profile update successfully.");
        setIsEditing(false);
      } else {
        setError(result.error || "Profile update failed. Plase try again.");
      }
    } catch (err) {
      setLoading(false);
