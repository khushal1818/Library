...
//   const exportCav = () => {
//     const cavContext = buildCsv();
//     const blob = new Blob([cavContext], { type: "text/csv;charset-utf-8;" });
...
//   //to confirm remove fine also return the book
...
//       } else {
//         triggerToast("Failed to clear fine. Pleased try again.", "error");
//       }
//     }
...
const AdminUserPage = () => {
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (statusFilter === "Overdue") {
      return sorted.sort((first, second) => {
        if (overdueSortOrder === "low-to-high") {
          return (first.totalFine ?? 0) - (second.totalFine ?? 0);
        }

        return (second.totalFine ?? 0) - (first.totalFine ?? 0);
      });
    }
  ...
  const exportCsv = () => {
    const csvContext = buildCsv();

    const blob = new Blob([csvContext], {
              ...
              <article key={member.email} className={s.studentCard}>
                ...
                <div className={s.statsRow}>
                  ...
                  <div className={s.statBlock}>
                    ...
                    <p className={s.numericStat}>{member.borrowedCount ?? 0}</p>
                  </div>

                  <div className={s.statBlock}>
                    <p className={s.statBlockLabel}>Total Fine</p>

                    <p className={s.numericStat}>Rs. {member.totalFine ?? 0}</p>
                  </div>
                  ...
                  <div className={s.statBlock}>
                    ...
                    <p className={s.numericStat}>
                      {member.fineClearedCount ?? 0}
                    </p>
                  </div>
                </div>
                  ...
                  <div className={s.expandedContainer}>
                    <div className={s.detailsCard}>
                      ...
                      <div className={s.detailsGrid}>
                        ...
                        <div className={s.detailsItem}>
                          Semester: {member.semester ?? "Semester 1"}
                        </div>
