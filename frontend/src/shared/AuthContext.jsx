...
// const SESSION_KEY = "library-auth-sessison";
...
//   // to logout and clear the localstotage
//   const logout = () => {
...
// Convert backend user into frontend user
const mapUserToFrontend = (user) => {
  if (!user) return null;
  ...
  return {
    ...
    // IMPORTANT:
    // MongoDB has rollNumber
    rollNumber: user.rollNumber || user.rollNo || user.rollno || "",

    studentId:
      user.studentId || (user._id ? `ST-${String(user._id).slice(-6)}` : ""),
...
export const AuthProvider = ({ children }) => {
  ...
  const fetchRegisteredUsers = async (token) => {
    if (!token) {
      console.warn("fetchRegisteredUsers: No token available");
      return;
    }
  ...
  const login = async ({ email, password, role }) => {
    try {
      ...
      return {
        ...
        error:
          "Server connection failed. Please ensure the backend is running on http://localhost:50000",
      };
    }
  };
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/complete-profile`, {
        ...
        body: JSON.stringify({
          ...
          // IMPORTANT:
          // Send rollNumber because your MongoDB field is rollNumber
          rollNumber,
        }),
      });
