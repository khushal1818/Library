...
// // dummy data so for frontend user or who wants the  data for data entery
// const defaultBooks = [
//   {
//     id: 1,
//     bookCode: "1001",
...
// const API_BOOKS_URL = "http://localhost:50000/api/books";
...
// dummy data so for frontend user or who wants the data for data entry
const defaultBooks = [
  {
    id: 1,
    bookCode: "1001",
...
|--------------------------------------------------------------------------
| IMPORTANT FIX:
| Catalog books issued from Books Page are converted into student history
| records here.
*/
const createCatalogRecord = (book, record, fineSettings) => ({
  ...record,
  ...
  department: record.department ?? book.department ?? "General",

  stream: record.stream ?? book.stream ?? "General",

  year: record.year ?? book.year ?? "1st Year",
...
export const LibraryProvider = ({ children }) => {
        ...
        return {
          ...
          borrowings: [
...

            {
              ...
              year: account.year ?? account.academicYear ?? "",
  ...
  const issueManualBooksToStudent = async ({
    ...
    try {
      ...
      return {
        ok: false,
        error: "Network error issuing manual books.",
      };
    }
  };
  ...
  const returnIssuedRecord = async ({ source, recordId, bookId = null }) => {
    ...
    // Update manual issues immediately
    setManualIssues((currentIssues) =>
      currentIssues.map((issue) =>
        issue.id === recordId || issue._id === recordId
          ? {
    ...
    // Update catalog book immediately
    if (source === "catalog" && bookId) {
      returnBook(bookId, recordId);
    }
  ...
  const saveFineSettings = async ({ amount, interval }) => {
    const updatedSettings = hydrateFineSettings({
      amount,
      interval,
    });
  ...
  | Previously:
  |
  | const allRecords = manualIssues.map(...)
  |
  | That meant books issued from the normal Books Page were NOT included
  | in Admin Users history.
  |
  | Now we collect:
  |
  | 1. Catalog book borrowings
  | 2. Manual/backend issues
  |
  |--------------------------------------------------------------------------
  */

  const catalogRecords = books.flatMap((book) =>
    Array.isArray(book.borrowings)
      ? book.borrowings.map((record) =>
          createCatalogRecord(book, record, fineSettings),
        )
      : [],
  );

  const manualRecords = manualIssues.map((issue) =>
    createManualRecord(issue, fineSettings),
  );

  // BOTH catalog + manual records
  const allRecords = [...catalogRecords, ...manualRecords];
  ...
  */
  ...
  const studentSummaries = studentAccounts.map((account) => {
    ...
    return {
      ...
      // THIS NOW CONTAINS CATALOG + MANUAL HISTORY
      history: records,
    };
  });
